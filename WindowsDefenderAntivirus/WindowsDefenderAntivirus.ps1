function WindowsDefenderAntivirus {
    ...
    if ($Disable) { # 關閉所有功能
        ...
        Write-Host "已關閉防毒軟體，但可能需要重新啟動才會生效。" -ForegroundColor:Yellow
        ...
        Write-Host "已關閉即時掃描，但新版 Windows 需要你手動關閉防竄改保護才會生效。" -ForegroundColor:Yellow
        Write-Host "  右下角系統圖示::Windows安全性" -ForegroundColor:Yellow
        Write-Host "     -> 病毒與威脅防護 -> 病毒與威脅防護設定::管理設定 -> 防竄改保護::關閉此項目" -ForegroundColor:Yellow
    } else { # 恢復為未設定狀態
